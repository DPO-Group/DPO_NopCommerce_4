...
namespace Nop.Plugin.Payments.Dpo.Controllers
{
    public class PaymentDpoController : BasePaymentController
    {
        ...
        private readonly String dpoLiveUrlV7 = "https://secure.3gdirectpay.com/API/v7/";
        ...
        public async void DpoNotifyHandler()
        {
                ...
                ))
            {
                ...
                var amount = navigator.SelectSingleNode("//TransactionAmount").Value;
                ...
                verified = order != null && amount == order.OrderTotal.ToString();
                ...
                switch (trans_status)
                {
                    ...
                    case "900":
                        query_status = PaymentStatus.Pending;
                        query_status_desc = "Transaction result could not be verified";
                        break;
        ...
        public async Task<IActionResult> DpoReturnHandler(IFormCollection form)
        {
            ...
            verifXML.Append($"<CompanyRef>{orderId}</CompanyRef>");
            verifXML.Append("</API3G>");
            ...
            while (!verified && cnt < 5)
            {
                ...
                payrequestId = navigator.SelectSingleNode("//TransactionRef").Value;
                var companyRef = navigator.SelectSingleNode("//CompanyRef").Value;
                var amount = navigator.SelectSingleNode("//TransactionAmount").Value;
            ...
            else
            {
                ...
                await _logger.ErrorAsync("DpoReturnHandler: Payment failed with the following description: " + query_status_desc);
                if (_orderProcessingService.CanCancelOrder(order))
                {
                    await _orderProcessingService.CancelOrderAsync(order, false);
                }
